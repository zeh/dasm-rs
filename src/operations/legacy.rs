...
pub unsafe extern "C" fn v_processor(mut str: *mut libc::c_char,
    ...
    let mut previousProcessor: Processors = st.execution.processor;
    st.execution.processor = Processors::None;
    ...
    match processorName.as_str() {
        "6502" => {
            ...
            st.execution.bitOrder = BitOrder::LeastMost;
            st.execution.processor = Processors::MOS_6502;
        },
        "6803" => {
            if !called {
                addhashtable(Mne6803.as_mut_ptr());
            }
            st.execution.bitOrder = BitOrder::MostLeast;
            st.execution.processor = Processors::MOTOROLA_6803;
        },
        "HD6303" | "hd6303" => {
            if !called {
                addhashtable(Mne6803.as_mut_ptr());
            ...
            st.execution.bitOrder = BitOrder::MostLeast;
            st.execution.processor = Processors::HD_6303;
        },
        "68705" => {
            if !called {
            ...
            st.execution.bitOrder = BitOrder::MostLeast;
            st.execution.processor = Processors::MOTOROLA_68705;
        },
        "68HC11" | "68hc11" => {
            if !called {
            ...
            st.execution.bitOrder = BitOrder::MostLeast;
            st.execution.processor = Processors::MOTOROLA_68HC11;
        },
        "F8" | "f8" => {
            if !called {
            ...
            st.execution.bitOrder = BitOrder::MostLeast;
            st.execution.processor = Processors::FAIRCHILD_F8;
        },
        _ => {
    ...
    called = true;
    if previousProcessor != Processors::None && st.execution.processor != previousProcessor {
...
pub unsafe extern "C" fn v_mnemonic(mut str: *mut libc::c_char,
    ...
    if st.execution.modeNext != AddressModes::None {
        /*	Force	*/
        addressMode = st.execution.modeNext;
        if (*mne).okmask & ((1) << addressMode as u8) as libc::c_ulong == 0 {
    ...
    if opidx == 2 {
        st.output.generated[0] = (opcode >> 8) as u8;
        st.output.generated[1] = opcode as u8;
    } else {
        st.output.generated[0] = opcode as u8;
    }
    match addressMode {
        AddressModes::BitMod => {
            sym = (*symbase).next;
        ...
        AddressModes::BitBraMod => {
            ...
            st.output.generated[opidx] = (*sym).value as u8;
            opidx = opidx + 1;
            sym = (*sym).next
        }
        AddressModes::Rel => { }
        _ => {
            ...
            if *Opsize.as_mut_ptr().offset(addressMode as isize) == 2 {
                if st.execution.bitOrder != BitOrder::LeastMost {
                    st.output.generated[(opidx - 1) as usize] = ((*sym).value >> 8) as u8;
                    st.output.generated[opidx] = (*sym).value as u8;
                    opidx = opidx + 1;
                } else {
                    st.output.generated[opidx] = ((*sym).value >> 8) as u8;
                    opidx = opidx + 1;
                }
            }
            sym = (*sym).next
    ...
    if (*mne).flags & 0x10 != 0 {
        if !sym.is_null() {
            ...
            st.output.generated[opidx as usize] = (*sym).value as u8;
            sym = (*sym).next
        } else {
        ...
        if sym.is_null() {
            ...
            if pcf & (SegmentTypes::Unknown | 2) == 0 {
                ...
                if dest >= 128 as libc::c_int as libc::c_long ||
                    ...
                    st.execution.redoIndex += 1;
                    st.execution.redoWhy |= ReasonCodes::BranchOutOfRange;
                    (*sym).flags =
            ...
            st.output.generated[(opidx - 1) as usize] = (dest & 0xff) as u8;
        }
    }
    st.output.generatedLength = opidx;
    generate();
    FreeSymbolList(symbase);
}
#[no_mangle]
        ...
        (*pIncfile).flags =
            ((*pIncfile).flags as libc::c_int & !(0x2 as libc::c_int)) as u8;
                  ...
                  strncmp(str, b"OFF\x00" as *const u8 as *const libc::c_char,
                          2) ==
                      0 {
        st.execution.listMode = ListMode::None
    } else {
        st.execution.listMode = ListMode::List
    };
}
...
pub unsafe extern "C" fn v_incbin(mut str: *mut libc::c_char,
    ...
    if !binfile.is_null() {
        if st.execution.redoIndex != 0 {
            ...
            fseek(binfile, 0,
                  2);
            st.output.generatedLength = ftell(binfile) as usize;
            generate();
            /* does not access st.output.generated[] if Redo is set */
        } else {
            loop  {
                st.output.generatedLength =
                    fread(
                        st.output.generated.as_mut_ptr() as *mut libc::c_void,
                        1,
                        ::std::mem::size_of::<[libc::c_uchar; 1024]>() as libc::c_ulong,
                        binfile
                    ) as usize;
                if st.output.generatedLength <= 0 {
                    break;
                }
                generate();
            }
...
pub unsafe extern "C" fn v_seg(mut str: *mut libc::c_char, mut _dummy: *mut _MNE) {
    ...
    for (index, seg) in &mut st.other.segments.iter().enumerate() {
        if strNew.eq(&seg.name) {
            st.other.currentSegment = index;
            programlabel();
            return;
        }
    }
    ...
    if st.execution.modeNext == AddressModes::BSS {
        seg.flags = seg.flags | SegmentTypes::BSS;
    }
    st.other.segments.insert(0, seg);
...
pub unsafe extern "C" fn v_hex(mut str: *mut libc::c_char,
    ...
    while *str.offset(i as isize) != 0 {
        if !(*str.offset(i as isize) as libc::c_int == ' ' as i32) {
            ...
            st.output.generated[st.output.generatedLength] = result;
...
pub unsafe extern "C" fn gethexdig(mut c: libc::c_int) -> libc::c_int {
    ...
    filesystem::writeln_to_file_maybe(st.output.listFile, "(Must be a valid hex digit)");
    return 0;
}
#[no_mangle]
...
pub unsafe extern "C" fn v_dc(mut str: *mut libc::c_char,
    ...
    while !sym.is_null() {
        ...
        if (*sym).flags as libc::c_int & 0x1 as libc::c_int != 0 {
            st.execution.redoIndex += 1;
            st.execution.redoWhy |= ReasonCodes::DCNotResolved
        }
        if (*sym).flags as libc::c_int & 0x8 as libc::c_int != 0 {
            ...
            loop  {
                ...
                if vmode != 0 {
                    ...
                    if (*tmp).flags as libc::c_int & 0x1 as libc::c_int != 0 {
                        st.execution.redoIndex += 1;
                        st.execution.redoWhy |= ReasonCodes::DVNotResolvedProbably
                    }
                    FreeSymbolList(tmp);
                }
                match st.execution.modeNext {
                    AddressModes::WordAdr => {
                        if st.execution.bitOrder != BitOrder::LeastMost {
                            st.output.generated[st.output.generatedLength] = (value >> 8 & 0xff) as u8;
                            st.output.generatedLength += 1;
                            st.output.generated[st.output.generatedLength] = (value & 0xff) as u8;
                            st.output.generatedLength += 1;
                        } else {
                            st.output.generated[st.output.generatedLength] = (value & 0xff) as u8;
                            st.output.generatedLength += 1;
                    ...
                    AddressModes::Long => {
                        if st.execution.bitOrder != BitOrder::LeastMost {
                            st.output.generated[st.output.generatedLength] = (value >> 24 & 0xff) as u8;
                            st.output.generatedLength += 1;
                            st.output.generated[st.output.generatedLength] = (value >> 16 & 0xff) as u8;
                            st.output.generatedLength += 1;
                            st.output.generated[st.output.generatedLength] = (value >> 8 & 0xff) as u8;
                            st.output.generatedLength += 1;
                            st.output.generated[st.output.generatedLength] = (value & 0xff) as u8;
            ...
            if vmode != 0 {
                ...
                if (*tmp).flags as libc::c_int & 0x1 as libc::c_int != 0 {
                    st.execution.redoIndex += 1;
                    st.execution.redoWhy |= ReasonCodes::DVNotResolvedCould
                }
                FreeSymbolList(tmp);
            }
            match st.execution.modeNext {
...
pub unsafe extern "C" fn v_ds(mut str: *mut libc::c_char,
    ...
    if st.execution.modeNext == AddressModes::WordAdr { mult = 2 }
    if st.execution.modeNext == AddressModes::Long { mult = 4 }
    programlabel();
    sym = eval(str, 0);
    if !sym.is_null() {
        ...
        if (*sym).flags as libc::c_int & 0x1 as libc::c_int != 0 {
            st.execution.redoIndex += 1;
            st.execution.redoWhy |= ReasonCodes::DSNotResolved
        } else {
            if !(*sym).next.is_null() &&
...
pub unsafe extern "C" fn v_org(mut str: *mut libc::c_char,
    ...
    if !(*sym).next.is_null() {
        st.output.orgFill = (*(*sym).next).value as u8;
...
pub unsafe extern "C" fn v_align(mut str: *mut libc::c_char,
    ...
    if !(*sym).next.is_null() {
        if (*(*sym).next).flags & SymbolTypes::Unknown != 0 {
            st.execution.redoIndex += 1;
            st.execution.redoWhy |= ReasonCodes::AlignNotResolved
        } else {
    ...
    if rorg != 0 {
        if (currentSegment.rflags | (*sym).flags) & SegmentTypes::Unknown != 0 {
            st.execution.redoIndex += 1;
            st.execution.redoWhy |= ReasonCodes::AlignRelocatableOriginNotKnown
        } else {
        ...
        st.execution.redoIndex += 1;
        st.execution.redoWhy |= ReasonCodes::AlignNormalOriginNotKnown
    } else {
    ...
    */
    ...
    if (*lab).flags as libc::c_int & SymbolTypes::Unknown as libc::c_int == 0 {
        if (*sym).flags as libc::c_int & SymbolTypes::Unknown as libc::c_int != 0 {
            st.execution.redoIndex += 1;
            st.execution.redoWhy |= ReasonCodes::EquNotResolved
            ...
            printf(b"INFO: Label \'%s\' changed from $%04lx to $%04lx\n\x00"
                   ...
                   *Av.as_mut_ptr().offset(0 as isize),
                   (*lab).value, (*sym).value);
            st.execution.redoIndex += 1;
            st.execution.redoWhy |= ReasonCodes::EquValueMismatch
        }
    }
    (*lab).value = (*sym).value;
    (*lab).flags =
    ...
    if v > 0xffff as libc::c_int as libc::c_ulong {
        st.output.generated[st.output.generatedLength] = (v >> 24) as u8;
        st.output.generatedLength += 1;
        st.output.generated[st.output.generatedLength] = (v >> 16) as u8;
        st.output.generatedLength += 1;
    }
    st.output.generated[st.output.generatedLength] = (v >> 8) as u8;
    st.output.generatedLength += 1;
    st.output.generated[st.output.generatedLength] = v as u8;
    st.output.generatedLength += 1;
    FreeSymbolList(sym);
}
#[no_mangle]
...
pub unsafe extern "C" fn v_echo(mut str: *mut libc::c_char,
    ...
    filesystem::writeln_to_file_maybe(st.output.listFile, "");
}
#[no_mangle]
...
pub unsafe extern "C" fn v_if(mut str: *mut libc::c_char,
    ...
    if (*sym).flags != 0 {
        st.execution.redoIndex += 1;
        st.execution.redoWhy |= ReasonCodes::IfNotResolved;
        pushif(false);
        (*Ifstack).acctrue = 0;
...
pub unsafe extern "C" fn v_repeat(mut str: *mut libc::c_char,
    ...
    if (*rp).flags as libc::c_int != 0 {
        st.execution.redoIndex += 1;
        st.execution.redoWhy |= ReasonCodes::RepeatNotResolved
    }
    Reploop = rp;
    FreeSymbolList(sym);
    pushif(true);
...
pub unsafe extern "C" fn v_incdir(mut str: *mut libc::c_char, mut _dummy: *mut _MNE) {
    ...
    if st.execution.includeDirList.iter().find(|dir| dir.cmp(&&filename) == Ordering::Equal).is_none() {
        st.execution.includeDirList.push(filename);
    }
    // FIXME: new, end
    buf = getfilename(str);
    tail = &mut incdirlist;
...
pub unsafe extern "C" fn generate() {
    ...
    if st.execution.redoIndex == 0 {
        if currentSegment.flags & SegmentTypes::BSS == 0 {
            i = st.output.generatedLength as i32 - 1;
            while i >= 0 {
                CheckSum = CheckSum.wrapping_add(st.output.generated[i as usize] as libc::c_ulong);
                i -= 1;
            }
            if st.execution.isClear {
                st.execution.isClear = false;
                if currentSegment.flags & SegmentTypes::Unknown != 0 {
                    st.execution.redoIndex += 1;
                    st.execution.redoWhy |= ReasonCodes::Obscure;
                    return
                }
                org = currentSegment.org;
                if st.parameters.format == Format::Default || st.parameters.format == Format::Ras {
                    ...
                    if st.parameters.format == Format::Ras {
            ...
            match st.parameters.format {
                Format::Raw | Format::Default => {
                    ...
                    while currentSegment.org != org {
                        putc(st.output.orgFill as libc::c_int, FI_temp);
                        org = org.wrapping_add(1)
                    }
                    fwrite(st.output.generated.as_mut_ptr() as *const libc::c_void,
                           st.output.generatedLength as size_t, 1 as size_t,
                           FI_temp);
                }
                Format::Ras => {
                    ...
                    fwrite(st.output.generated.as_mut_ptr() as *const libc::c_void,
                           st.output.generatedLength as size_t, 1 as size_t,
                           FI_temp);
                    Seglen += st.output.generatedLength;
                }
            }
            org = org.wrapping_add(st.output.generatedLength as u64)
        }
    }
    currentSegment.org = currentSegment.org.wrapping_add(st.output.generatedLength as u64);
    if currentSegment.flags & SegmentTypes::RelocatableOrigin != 0 {
        currentSegment.rorg = currentSegment.rorg.wrapping_add(st.output.generatedLength as u64)
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn genfill(mut fill: libc::c_long,
    ...
    match size {
        1 => {
            memset(st.output.generated.as_mut_ptr() as *mut libc::c_void, c0 as libc::c_int,
        ...
        2 => {
            ...
            while (i as libc::c_ulong) <
                      ::std::mem::size_of::<[libc::c_uchar; 1024]>() as
                          libc::c_ulong {
                if st.execution.bitOrder != BitOrder::LeastMost {
                    st.output.generated[(i + 0) as usize] = c1;
                    st.output.generated[(i + 1) as usize] = c0
                } else {
                    st.output.generated[(i + 0) as usize] = c0;
                    st.output.generated[(i + 1) as usize] = c1
                }
                i += 2
            }
        }
        4 => {
            ...
            while (i as libc::c_ulong) <
                      ::std::mem::size_of::<[libc::c_uchar; 1024]>() as
                          libc::c_ulong {
                if st.execution.bitOrder != BitOrder::LeastMost {
                    st.output.generated[(i + 0) as usize] = c3;
                    st.output.generated[(i + 1) as usize] = c2;
                    st.output.generated[(i + 2) as usize] = c1;
                    st.output.generated[(i + 3) as usize] = c0
                } else {
                    st.output.generated[(i + 0) as usize] = c0;
                    st.output.generated[(i + 1) as usize] = c1;
                    st.output.generated[(i + 2) as usize] = c2;
                    st.output.generated[(i + 3) as usize] = c3
                }
                i += 4
            }
        }
    ...
    st.output.generatedLength = ::std::mem::size_of::<[libc::c_uchar; 1024]>();
    ...
    st.output.generatedLength = bytes as usize;
    generate();
}
#[no_mangle]
